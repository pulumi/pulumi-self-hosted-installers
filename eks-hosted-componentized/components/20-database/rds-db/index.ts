...
export class RdsDatabase extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.password = new random.RandomPassword(
      ...
      { additionalSecretOutputs: ["result"], parent: defaultParent }
    ).result;
    ...
    this.dbSubnets = new aws.rds.SubnetGroup(
      `${name}-subnets`,
      {
    ...
    const finalSnapshotIdentifier = new random.RandomId(
    ...
    let databaseInstanceOptions = new aws.rds.ParameterGroup(
      "database-instance-options",
      {
        ...
        parameters: [
          ...
          // Set SQL mode to STRICT_TRANS_TABLES for strict data validation
    ...
    let databaseClusterOptions = new aws.rds.ClusterParameterGroup(
    ...
    this.db = new aws.rds.Cluster(
      `${name}-cluster`,
      {
        ...
        dbClusterParameterGroupName: databaseClusterOptions.name,
        engine: engine,
        engineVersion,
        ...
        vpcSecurityGroupIds: pulumi
          .output(args.securityGroupId)
          .apply((id) => [id]), // Must be able to communicate with EKS nodes.
        finalSnapshotIdentifier: finalSnapshotIdentifier.hex,
        tags,
      },
      { protect: true, parent: defaultParent }
    );
