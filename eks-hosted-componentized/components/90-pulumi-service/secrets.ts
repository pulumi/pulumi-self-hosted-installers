...
export class SecretsCollection extends ComponentResource {
  ...
  constructor(
    name: string,
    args: SecretsCollectionArgs,
    opts?: ComponentResourceOptions
  ) {
    super("x:kubernetes:secrets", name, opts);

    this.LicenseKeySecret = new k8s.core.v1.Secret(
      `${args.commonName}-license-key`,
      {
    ...
    this.DBConnSecret = new k8s.core.v1.Secret(
      `${args.commonName}-mysql-db-conn`,
      {
        metadata: {
    ...
    this.SmtpSecret = new k8s.core.v1.Secret(
      `${args.commonName}-smtp-secret`,
      {
        metadata: {
          namespace: args.namespace,
        },
    ...
    this.RecaptchaSecret = new k8s.core.v1.Secret(
      `${args.commonName}-recaptcha-secret`,
      {
        metadata: {
          namespace: args.namespace,
        },
    ...
    this.OpenSearchSecret = new k8s.core.v1.Secret(
      `${args.commonName}-opensearch-secrets`,
      {
        metadata: {
          namespace: args.namespace,
        },
    ...
    this.GithubSecret = new k8s.core.v1.Secret(
      `${args.commonName}-github-secrets`,
      {
        metadata: {
          namespace: args.namespace,
        },
    ...
    // SSO related secrets
    const ssoPrivateKey = new tls.PrivateKey("ssoPrivateKey", {
      algorithm: "RSA",
      rsaBits: 2048,
    });
    ...
    this.SamlSsoSecret = new k8s.core.v1.Secret(
      `${args.commonName}-saml-secrets`,
      {
        metadata: {
          namespace: args.namespace,
        },
