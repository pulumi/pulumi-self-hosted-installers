...
2.1 | September, 2026 | -- | Moves the 02-kubernetes search cluster from OpenSearch 2.x to 3.7.0. See the OpenSearch upgrade procedure below.
...
This is a disruptive update to the search cluster in 02-kubernetes but does NOT destroy any stateful resources
(e.g. the MySQL server, or the buckets). Allow for about thirty minutes, plus the time it takes to reindex.

This installer deploys the OpenSearch chart with `persistence` disabled, so the pods hold their indices in ephemeral
storage and any pod replacement discards them. The search cluster is not stateful and can be reindexed on demand, so
this costs a service disruption but no permanent data loss - it just means a reindex is required afterwards.

Unlike the EKS installer, this one deploys no OpenSearch Dashboards chart, and the OpenSearch StatefulSet and Service
selectors are identical between 2.x and 3.7.0. There is no immutable field to work around, so `pulumi up` upgrades
...
* 01-infrastructure
  * SKIP
* 02-kubernetes
  * Run `npm install` to pick up the package versions pinned by this release.
  * Run `pulumi up`
    * This rolls the `opensearch-cluster-master` StatefulSet onto the 3.7.0 image.
    * The admin password is generated by this stack and exported as `openSearchPassword`, so nothing needs to be set
      by hand. OpenSearch dropped the built-in `admin`/`admin` account in 2.12 and the chart takes the generated
      value through `OPENSEARCH_INITIAL_ADMIN_PASSWORD`.
    * The cluster runs on GKE Autopilot with `sysctlInit` disabled, so `vm.max_map_count` is set by the
      `max-map-count-setter` DaemonSet instead. 3.7.0 wants the same 262144 as 2.x, so that DaemonSet is unchanged.
* 03-application
  * SKIP

Once the pods are running again (`kubectl -n $(pulumi stack output appNamespace) get pods` from `02-kubernetes`),
log in to the service. The resources page will be empty until the search cluster is repopulated, so an admin should
go to Settings->Self-hosted and reindex it:
