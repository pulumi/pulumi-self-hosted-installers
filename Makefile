...
test-all:
	...
	cd tests && PULUMITEST_RETAIN_FILES_ON_FAILURE=false go test -v -timeout=2h -tags "aws,azure,gke,integration" ./...

# Individual platform tests
test-aws-eks:
	@echo "Running AWS EKS tests..."
	cd tests && go mod download
	cd tests && PULUMITEST_RETAIN_FILES_ON_FAILURE=false go test -v -timeout=2h -tags aws -run TestAwsEksTsExamples ./...

test-aws-ecs-ts:
	@echo "Running AWS ECS TypeScript tests..."
	cd tests && go mod download
	cd tests && PULUMITEST_RETAIN_FILES_ON_FAILURE=false go test -v -timeout=90m -tags aws -run TestAwsEcsTsExamples ./...

test-aws-ecs-go:
	@echo "Running AWS ECS Go tests..."
	cd tests && go mod download
	cd tests && PULUMITEST_RETAIN_FILES_ON_FAILURE=false go test -v -timeout=90m -tags aws -run TestAwsEcsGoExamples ./...

test-azure-aks:
	@echo "Running Azure AKS tests..."
	cd tests && go mod download
	cd tests && PULUMITEST_RETAIN_FILES_ON_FAILURE=false go test -v -timeout=90m -tags azure -run TestAzureAksTsExamples ./...

test-gke:
	@echo "Running GKE tests..."
	cd tests && go mod download
	cd tests && PULUMITEST_RETAIN_FILES_ON_FAILURE=false go test -v -timeout=90m -tags gke -run TestGkeTsExamples ./...

test-integration:
	@echo "Running parallel integration tests..."
	cd tests && go mod download
	cd tests && PULUMITEST_RETAIN_FILES_ON_FAILURE=false go test -v -timeout=2h -tags integration -run TestParallelIntegration ./...
