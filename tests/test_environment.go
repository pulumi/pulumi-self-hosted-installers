...
func (env *TestEnvironment) applyResourceTags(t *testing.T, pulumiTest *pulumitest.PulumiTest) {
	...
	// AWS tags (most platforms support these)
	for key, value := range env.ResourceTags {
		// Convert to AWS tag format
		awsTagKey := fmt.Sprintf("aws:tags:%s", strings.ReplaceAll(key, ":", "_"))
		pulumiTest.SetConfig(t, awsTagKey, value)
	}
...
// cleanupPulumiTest destroys a Pulumi stack
